...
exports.upsertRecruit = function (req, res) {
  ...
  return Recruit.findByIdAndUpdate(rec._id || '', rec, {new: true, upsert: true}, function (err, recruit) {
    if (!err) {
      if (recruit) {
        return res.json(recruit);
      }
      else {
        var newRec = new Recruit({
    ...
    Recruit.find(options, function (err, recs) {
      ...
      for (var i = 0; i < recs.length; i++) {
        ...
        if (!existingPlayer) {
          ...
          console.log('Recruit ' + rec.forename + ' ' + rec.surname + ' was already enrolled. Skipping...');
        }
      }
